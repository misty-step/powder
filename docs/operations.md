...
`POWDER_TAILNET_ADMIN` is false by default. Verified tailnet identities are ordinary non-admin actors unless an operator explicitly sets `POWDER_TAILNET_ADMIN=true`; this is a deliberate single-operator choice and should not be enabled for shared tailnets.
