...
- `powder-api` and `powder-cli` are thin faces over the same domain and shell
  contracts. No business rule may live only in an adapter. Agents use the CLI
  plus `SKILL.md`; HTTP serves the UI and integrations.
