...
# non-terminal (backlog) so the board's derived BLOCKED strip has a real row.
...
cargo run -q -p powder-cli -- create-card --db "$DB" --id blocker-dep --title "Dependency the blocked card waits on" >/dev/null
