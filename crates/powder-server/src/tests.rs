...
/// actually exposes the claim (status, claim id, runtime reference, and agent)
/// that depends on it.
#[tokio::test]
async fn claim_then_get_card_then_renew_round_trips_for_same_identity() {
    ...
    let claimed = app
        ...
        .oneshot(json_request(
            ...
            Some(&raw_key),
            r#"{"agent":"lane-x","runtime_ref":"bb-run-42","ttl_seconds":3600}"#,
        ))
        .await
        .unwrap();
    assert_eq!(claimed.status(), StatusCode::OK);
    let claimed = response_json(claimed).await;
    let claim_id = claimed["claim_id"].as_str().unwrap().to_string();
    ...
    assert_eq!(detail["card"]["claim"]["id"], claim_id);
    assert_eq!(detail["card"]["claim"]["runtime_ref"], "bb-run-42");
    ...
    let renewed = app
        .oneshot(json_request(
            ...
            Some(&raw_key),
            &format!(r#"{{"claim_id":"{claim_id}","ttl_seconds":3600}}"#),
        ))
        .await
        .unwrap();
