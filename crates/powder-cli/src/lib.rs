...
fn list_ready(args: &[String], remote_env: &RemoteEnv) -> Result<String, ShellError> {
    ...
    let ready = if let Some(db) = flag_value(args, "--db") {
        ...
        let page = store
            .list_ready_page_after(query.clone(), after.as_ref().map(|cursor| &cursor.anchor))
            .map_err(store_err)?;
...
mod tests {
    ...
    fn cli_list_ready_json_filters_and_returns_cursor_metadata() {
        ...
        run(&args(["init-db", "--db", &db])).unwrap();
        for (id, repo, risk) in [
            ("cli-ready-1", "repo-a", "low"),
            ("cli-ready-2", "repo-a", "low"),
            ("cli-ready-missing-risk", "repo-a", "high"),
            ("cli-ready-other", "repo-b", "low"),
        ] {
            run(&args([
                "create-card",
                "--db",
        ...
        assert_eq!(page["cards"][0]["id"], "cli-ready-1");
        assert_eq!(page["has_more"], true);
        assert!(page["next_after"].as_str().is_some());
