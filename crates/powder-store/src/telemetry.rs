...
impl PricingConfig {
    pub fn from_json_str(raw: &str) -> Result<Self> {
        ...
        if config
            .rates
            .iter()
            .any(|r| r.model.trim().is_empty() || r.version.trim().is_empty())
        {
...
impl Store {
    ...
    pub fn record_run_telemetry_with_pricing(
        ...
        if write.attempts.is_empty() && write.summary.is_none() {
            return Err(
                DomainError::validation("telemetry", "attempts or summary is required").into(),
            );
        }
    ...
    pub fn run_telemetry_aggregate(
        ...
        while let Some(row) = rows.next()? {
            ...
            let entry = grouped
                .entry((agent.clone(), model.clone(), provider.clone()))
                .or_insert_with(|| RunTelemetryAggregateRow {
                    unattributed: model == "(unattributed)" || provider == "(unattributed)",
                    agent,
                    model,
                    provider,
                    runs: 0,
                    ...
                    duration_ms: 0,
                    outcome_mix: std::collections::BTreeMap::new(),
                });
            entry.runs += row.get::<_, i64>(4)?;
            entry.attempts += row.get::<_, i64>(5)?;
            entry.input_tokens += row.get::<_, i64>(6)?;
            entry.output_tokens += row.get::<_, i64>(7)?;
            entry.reasoning_tokens += row.get::<_, i64>(8)?;
            entry.estimated_cost_usd_micros += row.get::<_, i64>(9)?;
            entry.duration_ms += row.get::<_, i64>(10)?;
            *entry
                .outcome_mix
                .entry(row.get::<_, String>(3)?)
                .or_default() += row.get::<_, i64>(5)?;
        }
...
fn record_in_transaction(
    ...
    let mut summary = write.summary.clone().unwrap_or_default();
    if !attempts.is_empty() {
        summary = RunTelemetrySummary::default();
        summary.attempt_count = attempts.len() as i64;
        ...
        for attempt in &mut attempts {
            ...
            summary.input_tokens = Some(
                summary
                    .input_tokens
                    .unwrap_or(0)
                    .saturating_add(attempt.input_tokens.unwrap_or(0)),
            );
            summary.output_tokens = Some(
                summary
                    .output_tokens
                    .unwrap_or(0)
                    .saturating_add(attempt.output_tokens.unwrap_or(0)),
            );
            summary.reasoning_tokens = Some(
                summary
                    .reasoning_tokens
                    .unwrap_or(0)
                    .saturating_add(attempt.reasoning_tokens.unwrap_or(0)),
            );
            summary.estimated_cost_usd_micros = Some(
                summary
                    .estimated_cost_usd_micros
                    .unwrap_or(0)
                    .saturating_add(attempt.estimated_cost_usd_micros.unwrap_or(0)),
            );
            summary.duration_ms = Some(
                summary
                    .duration_ms
                    .unwrap_or(0)
                    .saturating_add(attempt.duration_ms.unwrap_or(0)),
            );
            if let Some(v) = attempt.pricing_version.as_ref() {
                versions.insert(v.clone());
            }
            if let Some(v) = attempt.outcome.as_ref() {
                outcomes.insert(v.clone());
            }
            if attempt.provider.is_none() || attempt.model.is_none() || attempt.harness.is_none() {
                summary.unattributed_attempt_count += 1;
            }
    ...
    summary.attempt_count = summary.attempt_count.saturating_add(old.0.unwrap_or(0));
    summary.input_tokens = sum_option(old.1, summary.input_tokens);
    summary.output_tokens = sum_option(old.2, summary.output_tokens);
    summary.reasoning_tokens = sum_option(old.3, summary.reasoning_tokens);
    summary.estimated_cost_usd_micros = sum_option(old.4, summary.estimated_cost_usd_micros);
    summary.duration_ms = sum_option(old.5, summary.duration_ms);
    summary.unattributed_attempt_count = summary
        .unattributed_attempt_count
        .saturating_add(old.8.unwrap_or(0));
    ...
    let audit_payload = serde_json::to_string(
        &json!({"run_id": run_id.as_str(), "attempt_count": summary.attempt_count, "unattributed_attempt_count": summary.unattributed_attempt_count, "pricing_version": summary.pricing_version}),
    )?;
...
fn sum_option(old: Option<i64>, next: Option<i64>) -> Option<i64> {
    match (old, next) {
...
fn calculate_cost(a: &RunTelemetryAttemptInput) -> i64 {
    let calc = |tokens: Option<i64>, rate: Option<i64>| -> i128 {
        i128::from(tokens.unwrap_or(0)) * i128::from(rate.unwrap_or(0)) / 1_000_000
    };
    (calc(a.input_tokens, a.input_rate_usd_per_million_micros)
        + calc(a.output_tokens, a.output_rate_usd_per_million_micros)
        + calc(a.reasoning_tokens, a.reasoning_rate_usd_per_million_micros))
