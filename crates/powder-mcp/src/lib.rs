...
pub const TOOLS: &[ToolDef] = &[
    ...
    ToolDef {
        name: "get_card",
        description: "Read one card with runs, activities, links, comments, claim state, and always-present claim_eligibility (eligible/code; message when ineligible; blockers only for unresolved_blockers) using the same rules as list_ready. A parent card also returns bounded child summaries and a deterministic epic_state rollup packet. detail defaults to concise: newest-first, most recent 20 per history section plus totals/hint when truncated; detailed returns full history.",
